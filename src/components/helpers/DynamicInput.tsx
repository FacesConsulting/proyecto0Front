...
const DynamicInput = ({ formikProps }: ProfessionalInformationProps) => {
  const handleChange = (
    ...
    values[index].nombreEspecialidad = event.target.value // Accedemos al arreglo en el indice con el nombre y asignamos el valor
  ...
  const handleDeleteFile = (index: number) => {
    ...
    deleteFile[index].documentoEspecialidad = null // Eliminamor con el archivo de la especialidad
  ...
  const handleChangeFile = (
    ...
    if (inputElement && inputElement.files && inputElement.files.length > 0) {
      ...
      const values = [...formikProps.values.especialidad] // Copiamos el arreglo (especialidades) en la variable (values)
      values[index].documentoEspecialidad = file // Accedemos al arreglo en el indice con el nombre y asignamos el valor
            ...
            <Grid
              ...
              <div
                ...
                <input
                  type='file'
                  name='archivo'
                  disabled={Boolean(data.nombreEspecialidad === '')}
                  id='archivo'
                ...
                />
                {data.documentoEspecialidad === null ? (
                  <div className='flex flex-col items-center justify-center p-2 text-center'>
                    <svg
                      xmlns='http://www.w3.org/2000/svg'
                      width='24'
                      height='24'
                      fill={data.nombreEspecialidad === '' ? '#9ca3af' : '#2193ce'}
                      viewBox='0 0 24 24'>
                  ...
                  <div
                    ...
                    <div className='relative w-full h-full flex flex-col items-center text-center bg-gray-100 border rounded select-none'>
                      ...
                      {handleValidateTypeOfFile(data.documentoEspecialidad) ? (
                        <Image
                          className='absolute inset-0 z-0 object-cover w-full h-full border-4 border-white'
                          src={URL.createObjectURL(data?.documentoEspecialidad || new Blob())}
                          alt={data.nombreEspecialidad}
                          fill
                        />
                      ) : (
                        <div className='absolute w-full h-full text-gray-400 transform top-0 flex justify-center items-center -m-4'>
                          <IconSvg
                            color={
                              data.nombreEspecialidad === '' ? '#9ca3af' : '#70b9df'
                            }
                            size={30}
